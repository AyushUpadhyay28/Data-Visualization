function getCoalData(coal_svg){
d3.json('./final_data/aust.json')
.then(json => onGeoJsonLoadedCoal(json))
.catch(err => console.log('ERROR: ', err));

const onGeoJsonLoadedCoal = json => {
...
.then(dataJson => onDataJsonLoadedCoal(dataJson))
.catch(err => console.log('ERR: ', err));
}

const tooltipPathCoal = (width, height, offset, radius) => {
const left = -width / 2;
const right = width / 2;
...
const onDataJsonLoadedCoal = json => {

// Loading colour scheme.
const valueRange = json.reduce((r, s) => r ? [Math.min(r[0], s.Coal), Math.max(r[1], s.Coal)] : [s.Coal, s.Coal], null);
...
return stateData ? second_colour(stateData.Coal) : '#ccc';
})
...
// .attr('d', tooltipPathCoal(160, 120, 5, 5))
...
second_tooltip.append('path')
.attr('d', tooltipPathCoal(160, 40, 5, 5))
var row = "Coal";
second_tooltip.append('text')
.text(`Coal :`)
.attr('x', -70)
...
.shapeWidth(30)
.cells([0, 125, 250, 375, 500, 625, 750, 875])
.orient('vertical')
.scale(second_colour);
