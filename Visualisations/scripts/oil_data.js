function getOilData(coal_svg){
    d3.json('./final_data/aust.json')
    .then(json => onGeoJsonLoadedOil(json))
    .catch(err => console.log('ERROR: ', err));
    
    const onGeoJsonLoadedOil = json => {
    ...
    .then(dataJson => onDataJsonLoadedOil(dataJson))
    .catch(err => console.log('ERR: ', err));
    }
    
    const tooltipPathOil = (width, height, offset, radius) => {
    const left = -width / 2;
    const right = width / 2;
    ...
    const onDataJsonLoadedOil = json => {
    
    // Loading colour scheme.
    const valueRange = json.reduce((r, s) => r ? [Math.min(r[0], s.Oil), Math.max(r[1], s.Oil)] : [s.Oil, s.Oil], null);
    ...
    return stateData ? second_colour(stateData.Oil) : '#ccc';
    })
    ...
    // .attr('d', tooltipPathOil(160, 120, 5, 5))
    ...
    // });
     second_tooltip.append('path')
    .attr('d', tooltipPathOil(160, 40, 5, 5))
    var row = "Oil";
    second_tooltip.append('text')
    .text(`Oil :`)
    .attr('x', -70)
